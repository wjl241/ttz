	...
	<head>
		...
		<script>
			function aa(){
			    ...
			    $('input:radio').each(function(i,input){
			        var name = input.getAttribute('sex');
			        obj[name] = this.value;
			    })
				...
				//var urlprefix = 'http://testapi.jihes.com/jsp/ttz/tp/';
				 $.ajax({
					url: urlprefix+'dealTp',
			        data:obj,
			        type:'post',
			        success:function(result){
			        	 alert('提交成功');
			        	 if(sex == 0){
			        		 window.location.href=urlprefix+"showTp2?name="+realName;
			        	 }else{
			        		 window.location.href=urlprefix+"showTp3?name="+realName;
			        	 }
	...
	<body>
		...
		<section class="code">
			<div>
				<form>
					<label>
					男性 <input name="sex" type="radio"  id = "male" value="0" checked = "checked" />  	
					女性<input name="sex" type="radio" id = "female" value="1" />
					</label> 
			
					<label for="">姓名</label>
					<input type="text" name="name" class="form-control" style="width: 260px;display: inline-block;" />
